...
const appCss = "/interactive-portfolio-hub/assets/styles-Cop8cvmy.css";
...
const $$splitComponentImporter = () => import("./index-DsKRMP0k.js");
const Route2 = createFileRoute("/")({
  head: () => ({
    meta: [{
      title: "Anika Dewari — B.Tech CSE Portfolio & Weekly Learnings"
    }, {
      name: "description",
      content: "Portfolio of Anika Dewari, B.Tech CSE 6th sem. Projects, skills, and weekly Practical Employability Skills learnings."
    }, {
      property: "og:title",
      content: "Anika Dewari — Portfolio"
    }, {
      property: "og:description",
      content: "A window into my daily learning journey."
    }]
  }),
...
const basePath = "/interactive-portfolio-hub";
let history = null;
function initializeHistory() {
  ...
  history = createBrowserHistory({
    window,
    parseLocation: () => {
      const { pathname, search, hash, state } = window.location;
      let appPath = pathname;
      if (appPath === basePath) {
        appPath = "/";
      } else if (appPath.startsWith(basePath + "/")) {
        appPath = appPath.slice(basePath.length);
      }
      const href = `${appPath}${search}${hash}`;
...
const getRouter = () => {
  const routerHistory = initializeHistory();
